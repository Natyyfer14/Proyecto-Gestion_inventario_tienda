...
def ver_producto(inventario):
    """
    Pide el nombre de un producto y muestra su precio, stock y precio con descuento.
    """
...
def agregar_producto(inventario):
    """
    Permite añadir un nuevo producto al inventario con validación de datos.
    """
...
def listar_inventario(inventario, umbral):
    """
    Muestra todos los productos y genera una alerta si el stock está bajo el umbral.
    """
    if not inventario:
        print("\n -- Inventario Vacío --")
        return
...
def actualizar_stock(inventario):
    """
    Modifica la cantidad de stock de un producto existente.
    """
...
def guardar_inventario(inventario):
    """
    Guarda el estado actual del inventario en el archivo productos.csv.
    """
    datos_para_csv = []
    for nombre, producto in inventario.items():
...
def main():
    ...
    # Intenta cargar el inventario desde el CSV
    try:
    ...
    # Cargar datos del DataFrame al diccionario de objetos Producto
    ...
    while True:
        ...
        if opcion == "1":
            ver_producto(inventario) # <-- Tu función aquí
        elif opcion == "2":
            ...
            guardar_inventario(inventario) # Guardar antes de salir
